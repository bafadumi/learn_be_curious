...
def internal_error(error):
    return "error with imporvements"
...
def create():
    ...
    try:
        return render_template("create_improvements.html", system=system)
    except Exception as e:
        abort(500)
...
def edit():
    ...
    try:
        return render_template(
            "edit_improvements.html", improvement=improvement, system_id=system_id
        )
    except Exception as e:
        abort(500)
...
def edit_post():
    ...
    Improvement.query.filter_by(id=improvement_id).update(
        dict(name=name, description=description, user_id=current_user.id)
    )
